...
# Handle --assets-path: append bitmap data to binary file and record offset
assets_offset = 0
if args.assets_path is not None:
    assets_path = args.assets_path
    if assets_path.exists():
        assets_offset = assets_path.stat().st_size
    else:
        assets_path.parent.mkdir(parents=True, exist_ok=True)
        assets_offset = 0
    with open(assets_path, 'ab') as f:
        f.write(bytes(glyph_data))
    print(f"Wrote {len(glyph_data)} bytes to {assets_path} at offset {assets_offset}", file=sys.stderr)
...
if args.assets_path is not None:
    print(f"static const uint8_t {font_name}Bitmaps[] = {{}};")
    print()
else:
